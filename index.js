/**
 * Loopback Built-in Model Extender
 *
 * A module to extend Loopback built-in Model (User, AccessToken, etc.)
...
// Init logger
const debug = require('debug')('loopback:model-extender')

// Init module
let ModelExtender = module.exports = {}

/**
 * Extend list of defined models
 *
 * @param {Object} app Loopback app instance
 * @param {Object} options Loader options
 * @param {string | string[] | Object | Object[]} options.models Model name or list of model names
 * @param {Object} options.options Global options
 * @param {Object} options.options.folderPath Set folder path
 */
ModelExtender.extend = (app, {models, options}) => {
  // Init options
  if (!options)
    options = {}
 ...
 */
const extendModel = ModelExtender.extendModel = (Model, options) => {
  // Get model name
  ...
  if (options.filePath)
    ...
    let fileName = options.fileName || _.kebabCase(modelName.toLowerCase()) + '-x'
    // Resolve file path
